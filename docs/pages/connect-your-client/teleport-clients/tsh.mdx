...
<GuidedSteps>
  <GuidedStepItem
    id="login-to-teleport"
    title="Log into Teleport"
    description="Log into a Teleport cluster. This command retrieves the user's certificates and saves them into ~/.tsh/teleport.example.com"
  >
    ```bash
    $ tsh login --proxy=teleport.example.com
    ```
  </GuidedStepItem>
  <GuidedStepItem id="ssh-into-node" title="SSH into a Node as usual">
    ```bash
    $ tsh ssh user@node
    ```
  </GuidedStepItem>
  <GuidedStepItem
    id="tsh-ssh-arguments"
    title="`tsh ssh` takes the same arguments as the OpenSSH client"
  >
    ```bash
  ...
  <GuidedStepItem
    id="symlink-teleport"
    title="You can even create a convenient symlink"
  >
    ```bash
  ...
  <GuidedStepItem
    id="done-ssh"
    title="Done!"
    description="Now your 'ssh' command is calling Teleport's `tsh ssh`"
  >
    ```bash
    $ ssh user@host
    ```
  ...
  <GuidedStepItem
    ...
    description="This command removes SSH certificates from a user's machine"
  >
    ```bash
    $ tsh logout
    ```
