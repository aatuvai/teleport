...
<GuidedSteps>
  ## Local users
  Imagine you have two users, Alice and Bob, who have different access needs.

  <StepSection>Define roles for Alice and Bob</StepSection>

  <Step id="create-admin-role">
    Create your first role that defines access for Alice. For this exercise, we'll call it `alice`.
  </Step>
  <Step id="admin-specs">
    Alice needs access to the ssh login, and the kubernetes group `edit`
  </Step>
  <Step id="create-ubuntu-role">
    Add a role below for Bob's new role, which for this exercise we'll call `bob`
  </Step>
  <Step id ="ubuntu-specs">
    Bob needs access to the `ubuntu` SSH login and the Kubernetes group `view`.
  </Step>

  <StepSection>Create your new roles and invite Alice and Bob to the cluster</StepSection>

  <Step id="cli-create-roles">
    Create the roles in your cluster using the `roles.yaml`you created.
    </Step>
  <Step id="cli-invite-roles">
    Invite `alice` and `bob`, assign them the new roles you created for them.
  ...
  <Checkpoint 
    ...
    description="Ensure that the roles alice and bob have been created successfully from roles.yaml. Both roles should define SSH and Kubernetes group permissions as specified."
  >
    Here are some troubleshooting tips:
    - Ensure that the Teleport Agent is running on your server.
    - Check that both `tctl` and `tsh` are properly installed and configured.
  </Checkpoint>

  <StepSection>Create a role template</StepSection>

  <Step id="create-role-template">
    Create your first role template called. For this exercise we'll call it `devs`.
  </Step>
  <Step id="template-specs">
  ...
  <Step id="cli-create-template">
    Use `tctl` to create the role template.
  </Step>

  <StepSection>Update Alice's and Bob's users with traits</StepSection>
  
  <Step id="create-traits-alice">
    Create traits `login` and `kubernetes_group` for Alice.
  </Step>
  <Step id="create-traits-bob">
    Create traits `login` and `kubernetes_group` for Bob.
  </Step>
  <Step id="cli-create-traits">
    Update both users' entries with the `tctl create -f` command.
  </Step>
  <Checkpoint 
    title="Verify user update success" 
    description="Ensure that both Alice and Bob's user entries have been updated successfully with the new traits."
  >
    Here are some troubleshooting tips:
    - Ensure that the Teleport Agent is running on your server.
    - Check that both `tctl` and `tsh` are properly installed and configured.
  ...
  <File name="roles.yaml" icon="file">
    <CodeBlock stepId="create-admin-role">
    ...
    <CodeBlock stepId="admin-specs">
      ```yaml showLineNumbers=5
      spec:
        allow:
          logins: ['admin']
    ...
    <CodeBlock stepId="create-ubuntu-role">
      ```yaml showLineNumbers=19
      kind: role
      version: v7
      metadata:
        name: bob
    ...
    <CodeBlock stepId="ubuntu-specs">
      ```yaml showLineNumbers=24
      spec:
        allow:
          logins: ['ubuntu']
  ...
  <File name="devs.yaml" icon="file">
    <CodeBlock stepId="create-role-template">
      ```yaml showLineNumbers
      kind: role
      version: v7
      metadata:
    ...
    <CodeBlock stepId="template-specs">
      ```yaml showLineNumbers=5
      spec:
        allow:
  ...
  <File name="traits.yaml" icon="file">
    <CodeBlock stepId="create-traits-alice">
      ```yaml showLineNumbers
      kind: user
      version: v2
      metadata:
    ...
    <CodeBlock stepId="create-traits-bob">
      ```yaml showLineNumbers=11
      ---
      kind: user
      version: v2
      metadata:
  ...
  <File name="CLI" icon="terminal">
    <CodeBlock stepId="cli-create-roles">
      ```bash showLineNumbers
      $ tctl create -f roles.yaml
      ```
    </CodeBlock>
    <CodeBlock stepId="cli-invite-roles">
      ```bash showLineNumbers=2
      $ tctl users add alice --roles=alice
      $ tctl users add bob --roles=bob
      ```
    </CodeBlock>
    <CodeBlock stepId="cli-create-template">
      ```bash showLineNumbers=4
      $ tctl create -f devs.yaml
      ```
    </CodeBlock>
    <CodeBlock stepId="cli-create-traits">
