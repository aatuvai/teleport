...
[trusted clusters](../../zero-trust-access/deploy-a-cluster/trustedclusters.mdx) with their own
...
<EnterpriseFeatureCallout title="Enable enterprise SSH node recording">
  Recording at the SSH node is available in Teleport Enterprise. [Start a free trial](https://goteleport.com/signup/).
</EnterpriseFeatureCallout>

<EnterpriseFeatureBadge message="Recording sessions at the proxy service is only available in Teleport Enterprise.">
...
The Proxy Service then establishes its own connection to the destination server authenticating in one of two ways:

- For the Teleport SSH Service and legacy OpenSSH servers, the Proxy Service requires the client to provide its credentials through SSH Agent Forwarding so that these credentials can be reused to connect to the OpenSSH server.
  - Note that SSH Agent Forwarding must be allowed for any user trying to connect in this mode.
...
The use of Recording Proxy Mode across your cluster is no longer necessary to support
[OpenSSH servers](../../enroll-resources/server-access/openssh/openssh-agentless.mdx).
Recording Proxy Mode is automatically used for OpenSSH servers without the need for SSH
...
use cases, but many existing and new features will not be supported in this mode due
