...
[Teleport Configuration Reference](../config/config.mdx).
...
| mcp.session.listen_sse_stream | Streamable HTTP client has started to listening server notifications via an SSE stream. |
| mcp.session.invalid_http_request | The client has sent an HTTP request that Teleport could not process. |
...
Proxy Recording Mode is automatically used for OpenSSH servers without the need for SSH
...
When in Recording Proxy Mode, Teleport will check that the host certificate of any
