...
template: rfd
rfdUrl: https://github.com/gravitational/teleport/blob/master/rfd/0238-delegating-access-to-ai-workloads.md
sidebar_position: 1
---

## What is Delegation Flows?

Enabling a user to <Annotation title="Lending Access" text="“Lending access” means a user explicitly and temporarily authorizes a remote AI agent to act using their permissions, with all actions auditable as being performed by the AI on behalf of the user.">"lend" their access to a remote AI workload</Annotation>, to allow it to
...
than any particular end-user. This <Annotation title="Fully-Autonomous AI workloads" text="Teleport's non-human (bot) access model works well when an AI runs independently with its own identity and permissions, and actions are attributed only to the bot—not to any end user.">suits fully-autonomous AI workloads</Annotation>, but
isn't appropriate for workloads that can be controlled by an end-user as it
creates a surface for <Annotation title="Privilege Escalation Attacks" text="A security risk where a user influences an AI workload that has broader permissions than the user, causing it to perform actions the user is not personally authorized to do, effectively bypassing access controls.">privilege escalation attacks</Annotation> (e.g. talking the bot into
...
complete currently-manual <Annotation title="ClickOps" text="Manually performing operational tasks by logging in from a personal device and clicking through UIs or consoles to manage systems. In this requirement, these are the tasks employees want a remote “Digital Twin” to automate on their behalf.">"ClickOps"</Annotation> tasks for them. Employees would run a CLI
...
It is currently only possible to request access to an MCP Server <Annotation title="Lorem ipsum" text="Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.">"application"</Annotation>
