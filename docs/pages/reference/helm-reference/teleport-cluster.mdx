...
See the [Teleport HA Architecture page](../../installation/self-hosted/deployments/high-availability.mdx)
for more details.
...
| `kubernetes_service` | auth `Deployment` | Provides secure access to the Kubernetes<br/>cluster where the Teleport cluster is hosted. | [Enrolling Kubernetes clusters](../../enroll-resources/kubernetes-access/introduction.mdx) |
...
| `aws` | Leverages AWS managed services to store data. | [Running an HA Teleport cluster using an AWS EKS Cluster](../../installation/self-hosted/helm-deployments/aws.mdx) |
| `gcp` | Leverages GCP managed services to store data. | [Running an HA Teleport cluster using a Google Cloud GKE cluster](../../installation/self-hosted/helm-deployments/gcp.mdx) |
| `azure` | Leverages Azure managed services to store data. | [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](../../installation/self-hosted/helm-deployments/azure.mdx) |
| `scratch` (v12 and above) | Generates empty Teleport configuration. User must pass their own config. This is discouraged, use `standalone` mode with [`auth.teleportConfig`](#authteleportconfig) and [`proxy.teleportConfig`](#proxyteleportconfig) instead. | [Running a Teleport cluster with a custom config](../../installation/self-hosted/helm-deployments/custom.mdx) |
...
See the [PROXY Protocol security section](../../zero-trust-access/management/security/proxy-protocol.mdx) for more details.
...
See the [Teleport Configuration Reference](../deployment/config/config.mdx) for the list of supported fields.
...
[The SSO documentation](../../zero-trust-access/sso/sso.mdx) explains how to create
...
- [`local`](../../zero-trust-access/rbac-get-started/users.mdx) for local users
- [`passwordless`](../../zero-trust-access/authentication/passwordless.mdx) to enable by
...
[Disabling local auth is required for FedRAMP / FIPS](../../zero-trust-access/compliance-frameworks/fedramp.mdx).
...
See [the locking modes documentation](../../identity-governance/locking.mdx) for more
details.
...
[Can be used to forbid passwordless access to your cluster](../../zero-trust-access/authentication/passwordless.mdx)
...
For possible values, [see the Teleport Configuration Reference](../deployment/config/config.mdx).
...
`global.clusterDomain` sets the domain suffix used by the Kubernetes DNS service. 
...
See our [installation guide](../../installation/single-machine/docker.mdx) for information on
...
[PSP removal guide](../../installation/self-hosted/helm-deployments/migration-kubernetes-1-25-psp.mdx).
...
| `chartMode`  | Guide                                                                                                              |
|--------------|--------------------------------------------------------------------------------------------------------------------|
| `standalone` | [Getting Started - Kubernetes](../../installation/self-hosted/helm-deployments/kubernetes-cluster.mdx)            |
| `aws`        | [Running an HA Teleport cluster using an AWS EKS Cluster](../../installation/self-hosted/helm-deployments/aws.mdx)         |
| `gcp`        | [Running an HA Teleport cluster using a Google Cloud GKE cluster](../../installation/self-hosted/helm-deployments/gcp.mdx) |
| `azure`      | [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](../../installation/self-hosted/helm-deployments/azure.mdx) |
| `scratch`    | [Running a Teleport cluster with a custom config](../../installation/self-hosted/helm-deployments/custom.mdx)              |
...
[the prometheus-operator documentation](https://prometheus-operator.dev/docs/getting-started/introduction/)
...
<details>
<summary>Read this if using Kubernetes 1.23+ on EKS</summary>
...
`aws` settings are described in the AWS guide: [Running an HA Teleport cluster using an AWS EKS Cluster](../../installation/self-hosted/helm-deployments/aws.mdx)
...
The requirements are described in [the Athena backend documentation](../deployment/backends.mdx#athena)
...
`aws.accessMonitoring` configures the [Access Monitoring](../../identity-governance/access-monitoring.mdx)
feature of the Auth Service.

Using this features requires setting up specific AWS infrastructure as described
in [the AccessMonitoring configuration section](../../identity-governance/access-monitoring.mdx).
...
`gcp` settings are described in the GCP guide: [Running an HA Teleport cluster using a Google Cloud GKE cluster](../../installation/self-hosted/helm-deployments/gcp.mdx)

## `azure`

`azure` settings are described in the Azure guide: [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](../../installation/self-hosted/helm-deployments/azure.mdx)
...
<Admonition type="note" title="Installing cert-manager">
  ...
  and the relevant sections of the [AWS](../../installation/self-hosted/helm-deployments/aws.mdx) and [GCP](../../installation/self-hosted/helm-deployments/gcp.mdx) guides for more information.
</Admonition>
...
<Admonition type="note" title="Configuring an Issuer">
  ...
  of the [AWS](../../installation/self-hosted/helm-deployments/aws.mdx) and [GCP](../../installation/self-hosted/helm-deployments/gcp.mdx) guides for more information.
</Admonition>
...
See the [Teleport config file reference](../deployment/config/config.mdx) for more details on possible values for `extra_fields`.
