...
import googleCloudSvg from "@site/src/components/Icon/svg/google-cloud.svg";

## Management consoles and CLI tools

Secure your cloud APIs with Teleport. Control access to management consoles and CLI tools using the same RBAC system you trust.
Track every change with granular audit logs and enable Just-in-Time access so teams get permissions only when they need them—reducing risk from persistent admin accounts.

<EnrollmentMethods title="Cloud Provider APIs" variant="rows">
  <Method
  title="Amazon Web Services (AWS)"
  description="Centralize AWS access for the console, APIs, CLI, and SDKs by using AWS SSO. And authenticated users can be mapped directly to AWS roles by automatically receiving permissions."
  icon={awsSvg}
  >
    ...
    <Method title="AWS Console and CLI access via Teleport agent" href="./aws-console/">
      **Recommended when:**
        - Full API audit for all requests is required. 
    </Method>
    <Method title="AWS Console and CLI with OIDC integration" href="./awsoidc-integration/">
      **Recommended when:**
        - Full API audit for all requests is required.
