---
title: Teleport Identity Security
description: An overview of Teleport Identity Security.
videoBanner: Vg5R6vpZffg
labels:
 - identity-security
---

Teleport Identity Security unifies management of access policies across your infrastructure.
It hardens your access controls and displays up-to-date relationships and policies of all
users, groups, and computing resources.

It can help you answer questions like:

- What resources can a specific user access?
- What users can access a specific resource?
- What are the relationships between users, roles, and resources?

## Getting started with Identity Security

Identity Security is a separately licensed product and is available to Teleport Enterprise customers.
Access Graph is a capability of Identity Security that displays the relationships of
policies of users, groups, and computing resources.

To verify the availability of the Access Graph, ensure that the Teleport Identity Security icon is present in the navigation sidebar.

<Admonition type="note">
Note: For managed Enterprise customers, Identity Security is enabled by default.
If you are a self-hosted Teleport customer, you will need to [deploy the Access Graph Service](access-graph/access-graph.mdx) and ensure you have an updated
`license.pem` with Identity Security enabled to use it.
</Admonition>

## Identity Activity Center

(!docs/pages/includes/policy/identity-activity-center.mdx!)

### Identity Activity Center guides and supported integrations

- [Enable Identity Activity Center](access-graph/identity-activity-center.mdx)
- [Discover AWS Access Patterns and CloudTrail events with Teleport Identity Security](./integrations/aws-sync.mdx) guide.
- [Analyze and correlate Teleport Audit Logs](./integrations/teleport.mdx)
- [Analyze and correlate GitHub Audit Logs and Access Paths](./integrations/github.mdx) 
- [Analyze and correlate Okta Audit Logs and Access Paths](../identity-governance/okta/okta.mdx)

## Identity Security guides

<DocCardList />

(!docs/pages/includes/identity-security/integrations-is.mdx!)

(!docs/pages/includes/identity-security/integrations-mwi-1.mdx!)

(!docs/pages/includes/identity-security/integrations-mwi-2.mdx!)

(!docs/pages/includes/identity-security/integrations-mwi-3.mdx!)