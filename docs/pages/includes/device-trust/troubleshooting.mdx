...
device. [Download the macOS tsh
installer](../../installation/single-machine/macos.mdx) to fix the problem.
...
1. Verifying device profile data, such as data originated from an MDM service, against the
   actual device
...
* L7 load balancer: make sure it propagates the [X-Forwarded-For header](
  ../../reference/deployment/config/config.mdx#proxy-service)
* L4 load balancer: enable [PROXY protocol](
  ../../zero-trust-access/management/security/proxy-protocol.mdx)
