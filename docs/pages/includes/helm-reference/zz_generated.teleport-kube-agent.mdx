...
See [the join method reference](../../reference/deployment/join-methods.mdx) for the list of possible
...
the agent. See [the Teleport Application access documentation](../../enroll-resources/application-access/reference.mdx#configuration)
for more details.
...
Application access documentation](../../enroll-resources/application-access/application-access.mdx)
for more details.
...
<Admonition type="tip" title="Supported values">
  You can see a list of all the supported [values which can be used in a Teleport Database Service configuration here](../../reference/deployment/config/config.mdx#database-service).
</Admonition>
...
See the [Teleport Configuration Reference](../../reference/deployment/config/config.mdx) for the list of supported fields.
...
<Admonition type="note">
  ...
  For more information on how to set static/dynamic labels for Teleport services, see [labelling nodes and applications](../../zero-trust-access/rbac-get-started/labels.mdx).
</Admonition>
...
See the [Teleport config file reference](../../reference/deployment/config/config.mdx) for
...
`skipHooks` configures the chart to not render any hooks.
This can be used in CI flows where the chart is rendered and applied separately.

<Admonition type="danger">
After disabling the hooks, uninstalling the `teleport-kube-agent` chart
will no longer remove the agents state (stored in secrets named after the chart
pods). If you set this boolean to true, you are responsible for deleting the
secrets after uninstalling the chart.

Failure to do so might leave valid Teleport credentials in your cluster, or cause
issues when a new agent with the same name is deployed in the cluster.
</Admonition>
