...
import (
	...
	"github.com/gravitational/teleport/build.assets/tooling/cmd/config-ref-generator/reference"
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Cannot open configuration reference template at %v: %v\n", tmplPath, err)
		os.Exit(1)
	}

	err = reference.Generate(teleportPackagePrefix, genconf, tmpl)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Could not generate the configuration reference: %v\n", err)
		os.Exit(1)
	}
}
