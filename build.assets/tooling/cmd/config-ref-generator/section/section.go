...
type DeclarationInfo struct {
	...
	// corresponding full source file path.
...
type SourceData struct {
	...
	// uses to format documentation for dynamic section configuration fields.
...
func NewSourceData(prefix string, source string) (SourceData, error) {
	...
	if filepath.Ext(source) != ".go" {
		return SourceData{}, fmt.Errorf("source path %v is not a Go file", source)
	}
	...
	if err != nil {
		return SourceData{}, fmt.Errorf("opening source file %v: %w", source, err)
	}
	defer f.Close()
	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, source, f, parser.ParseComments)
	if err != nil {
		return SourceData{}, fmt.Errorf("parsing source file %v: %w", source, err)
	}
	...
	for _, imp := range file.Imports {
		...
		if err != nil {
			return SourceData{}, fmt.Errorf("reading %v: %w", pkgDir, err)
		}
...
func registerDeclsFromFile(prefix string, source string, typeDecls map[PackageInfo]DeclarationInfo) error {
...
type rawField struct {
	...
	name string
	// Name as it appears in YAML, based on the "yaml" struct tag and
	// marshaling rules in the gopkg.in/yaml.v2 package.
	yamlName string
...
func typeForDecl(decl DeclarationInfo, allDecls map[PackageInfo]DeclarationInfo) (rawType, error) {
	...
	for _, field := range str.Fields.List {
		...
		yamlName := getYAMLTag(f.tags)
		// This field is ignored, so skip it.
		// See: https://pkg.go.dev/gopkg.in/yaml.v2#Marshal
		if yamlName == "-" {
			continue
		}
		// Using the exported field declaration name as the field name
		// per YAML marshaling rules.
		if yamlName == "" {
			f.yamlName = f.name
		}
...
func makeYAMLExample(fields []rawField) (string, error) {
	...
	for _, field := range fields {
		...
		buf.WriteString(getYAMLTag(field.tags) + ": ")
...
// Key-value pair for the "yaml" tag within a struct tag.  See:
// https://pkg.go.dev/reflect#StructTag
var yamlTagKeyValue = regexp.MustCompile(`yaml:"([^"]+)"`)

// getYAMLTag returns the "yaml" tag value from the provided struct tag
// expression.
func getYAMLTag(tags string) string {
	kv := yamlTagKeyValue.FindStringSubmatch(tags)
...
// suitable as a section within the configuration reference.
...
func makeRawField(field *ast.Field, packageName string, allDecls map[PackageInfo]DeclarationInfo, namedImports map[string]string) (rawField, error) {
	...
	kind, err := getYAMLTypeForExpr(field.Type, packageName, allDecls, namedImports)
	if err != nil {
		return rawField{}, err
	}
...
assignTag:
	...
	return rawField{
		...
		kind:        kind,
		name:        name,
		yamlName:    getYAMLTag(tag),
		tags:        tag,
	}, nil
}

// makeFieldTableInfo assembles a slice of human-readable information about
// fields within a Go struct to include within the configuration reference.
...
func printableTitle(ident, yamlName string, camelCaseExceptions []string, titleWordReplacements []map[string]string) string {
	if yamlName == "" {
		return splitCamelCase(ident, camelCaseExceptions)
	}
...
func getSectionYAMLName(allDecls map[PackageInfo]DeclarationInfo, typeName string) (string, bool) {
	var fileConfigDecl *DeclarationInfo
	...
	fcType, err := typeForDecl(*fileConfigDecl, allDecls)
	if err != nil {
		return typeName, false
	}
	for _, f := range fcType.fields {
		custom, ok := f.kind.(yamlCustomType)
		if ok && custom.name == typeName {
			return f.yamlName, true
		}
	}
	return typeName, false
}
...
func ReferenceDataFromDeclaration(
	...
	entry := ReferenceEntry{
		...
		Description: printableDescription(description, rs.name, sectionYAMLName),
